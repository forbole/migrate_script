...
async function NewMsgTable() {
  console.log("CREATE TABLE message_backup_new");

  await query(`CREATE TABLE message_backup_new
    (
      ...
      CREATE INDEX message_backup_new_transaction_hash_index ON message_backup_new (transaction_hash);
      CREATE INDEX message_backup_new_type_index ON message_backup_new (type);
      CREATE INDEX message_backup_new_involved_accounts_index ON message_backup_new (involved_accounts_addresses);
      GRANT ALL PRIVILEGES ON message_backup_new TO  ${PGUSER};`)
}
...
//     "limit" BIGINT = 100,
//     "offset" BIGINT = 0
//   ) RETURNS SETOF message_backup_new AS $$
//   SELECT
